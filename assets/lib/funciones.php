...
////////////////////////////////////////////////////
function validaUsuario($lo,$pa,$conectar)
{
    $x = false;
    $link=conectar();
    $sql="SELECT * FROM usuarios where usuario = '$lo' and pass =md5('$pa')  " ;
...
function restaDosFechasold($minuendo, $sustraendo)
{
	//Separo los d�as, meses y a�os de las dos fechas
	list( $dia1, $mes1, $ano1) = explode( '[/.-]', $minuendo );
	list( $dia2, $mes2, $ano2) = explode( '[/.-]', $sustraendo );
...
function cargar_selectnormal($tabla,$conectar)
{

    $sql = "select * from " . $tabla . " " . "ORDER BY 2 ASC";
    $res = mysql_query($sql, $conectar) or die (mysql_error());
...
function cargaOcupacion($conectar)
{

    $a=array();
    $x=0;
    $sql="SELECT id, ocupacion FROM ocupacion";
    $res=mysql_query($sql, $conectar);
    while($f=mysql_fetch_array($res))
    {
        $a[$x]=$f;
...
function tratamientoGes($conectar)
{

    $a=array();
    $x=0;
    $sql="SELECT id, descripcion FROM tratamientoGes";
    $res=mysql_query($sql, $conectar);
    while($f=mysql_fetch_array($res))
    {
        $a[$x]=$f;
...
function restaDosFechas($endDate, $beginDate){
