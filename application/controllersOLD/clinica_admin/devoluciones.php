...
class Devoluciones extends CI_Controller {
    ...
    public function listarDevolucion()
    {
        $data['datos']      = $this->bancos_model->dameCtas();
        $data['breadcumb']  = "Registros";
    ...
    public function FiltroListarDevolucion()
    {
        ...
        $data['datos']      = $this->bancos_model->dameCtas($filtro,$fechaDesde,$fechaHasta);
        $data['breadcumb']  = "Registros";
        $data['title']      = "Listado de Cuentas para Devoluciones";
        Layout_Helper::cargaVista($this,'listar_devoluciones',$data,'ingresos');   
        //Layout_Helper::cargaVista($this,'../ingresos/listar_devoluciones',$data,'ingresos');   
    }
     * */
    ...
    ////GUARDAR DATOS DE DEVOLUCION
        $bancos = $this->bancos_model->dameUno($idFicha);
            ...
            $this->bancos_model->ctaNomPaciente  = $paciente->apellidoPaterno.' '.$paciente->apellidoMaterno.' '.$paciente->nombres;
    ...
    public function agregarDeposito($id)
    {
        ...
        IF(empty($apoderado)){
            echo "<script>
                ...
                if (c === true){
                    window.location.href='".base_url()."clinica_admin/ingresos/modificarRegistro/".$id."';
                        }
                else {
                    window.location.href='".base_url()."clinica_admin/devoluciones/listarIngreso';
                }
                </script>";
        }
    ...
    ////GUARDAR DATOS DE DEPOSITO  
        $dep = $this->bancos_model->dameUltimoDeposito($idFicha);
        
        $this->bancos_model->depId = $dep->depId;
        $this->bancos_model->depEstado = 2;
        $this->bancos_model->guardarDeposito();
        unset($this->bancos_model->depId,$this->bancos_model->depEstado);
        ...
        redirect(base_url().'clinica_admin/devoluciones/listarDevolucion');
    }
