...
class Apoderados_model extends CI_Model
{
     ...
     public function dameTodo($filtro='')
    {
        ...
        $data =  $this->db->distinct('p.apoId')
                    ->select('p.apoId id, p.apoRut rut , p.apoNombre nombres, p.apoApePat apellidoPaterno, p.apoApeMat apellidoMaterno, p.apoDireccion direccion, p.apoTelefono telefono, p.apoCelular celular, p.apoEmail email, p.apoFichaElectronica registro, p.apoTipo, a.nombres pNombre, a.apellidoPaterno pApellido')//, c.id comId, c.comuna comNombre')//i.isapre isaNombre, i.id isaId, 
                    ->from('apoderados p')
                    ->join('ficha_ugh_registro r','r.id=p.apoFichaElectronica','left')
                    ->join('ficha_pacientes a','r.paciente=a.id','left')
                    ->get()
                    ->result();
        }
        else{
