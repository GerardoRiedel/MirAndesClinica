    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 bhoechie-tab-container" >
                <br>
                <i class="fa fa-table"  style="color:#1d7044;font-size:18px; margin-left:15px" title="Exportar Tabla a Excel"> </i><span id="btnExport"style="cursor: pointer; color:#1d7044;font-size: 14px;"> <b>Export</b></span>
                    <br>
                <div  class="col-lg-12" style="overflow: auto;" id="exportar">
                ...
                <table class='table table-bordered table-hover table-striped '>
                        ...
                        <tbody>
                            ...
                            <?php IF($item->insId != 35 && $item->insId != 36 && $item->insId != 37 && $item->insId != 38 && $item->insId != 39){ ?>
                            <tr>
                                <td><?php echo $item->insId; ?></td>
                                <td><?php echo $item->insNombre; ?></td>
                                <td><?php echo $item->insValor; ?></td>
                                <td><?php IF($item->insEstado==='1')echo 'Activo';ELSE echo 'Desactivo'; ?></td>
                                <td><?php echo $item->insStock; ?></td>
                                <td>
                                    <?php $diferencia = 0; $color= '';
                                        FOREACH($stock as $sto){ ?>
                                        <?php //echo $sto->inaInsumo.'ddd'.$item->insId.'ss';
                                            IF($sto->inaInsumo === $item->insId){
                                                $contar = $sto->contar; echo $contar;
                                                $stockContar = $item->insStock;IF(empty($stockContar))$stockContar=0;
                                                $diferencia = $stockContar-$contar;
                                                IF($diferencia<=0)$color='style="color:red"';
                                                ELSE $color='';
                                        ...
                                        ?>
                                    <?php } 
                                    IF(empty($item->insStock)|| $item->insStock===0)$color='style="color:red"'
                                    ?>
                                </td>
                                <td <?php echo $color; ?>><?php IF($diferencia!=0)echo $diferencia; ELSEIF($diferencia===0)echo $item->insStock;$diferencia = 0?></td>
                                <td></td>
                            </tr>
                            <?php } ?>
                                <?php endforeach; ?>
...
<script>
    $("#btnExport").click(function(e) {
        ...
        tmpElemento.download = 'Stock de Insumos '+hoy+'.xls';
