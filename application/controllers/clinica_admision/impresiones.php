    ...
    public function imprimirImprimir()
    {
        ...
        IF(!empty($licencia)){
            IF(!empty($licencia->licDesde) && !empty($licencia->licDias)){
                ...
                $mes                = date ( 'm' , $nuevafecha );if($mes==='10')$mes='Octubre';elseif($mes==='11')$mes='Noviembre';elseif($mes==='12')$mes='Diciembre';elseif($mes==='01')$mes='Enero';elseif($mes==='02')$mes='Febrero';elseif($mes==='03')$mes='Marzo';elseif($mes==='04')$mes='Abril';elseif($mes==='05')$mes='Mayo';elseif($mes==='6')$mes='Junio';elseif($mes==='07')$mes='Julio';elseif($mes==='08')$mes='Agosto';elseif($mes==='09')$mes='Septiembre';
...
///TENGO QUE BUSCAR EL ULTIMO DEPOSITO       
        ...
        $tec = $this->input->post('formTec');
        IF($tec === 'on'){
            $opciones[]= '';
            $opciones[10]= $this->input->post('formTec');
            $opciones[11]= $this->input->post('formInsumos');
        }
        ELSE $opciones[10]= 'off';
...
///TENGO QUE BUSCAR EL ULTIMO DEPOSITO       
    ...
    public function imprimirImprimirPrefactura($pref)
    {
        $prefacturas = $this->prefacturas_model->dameUnaPrefClinica($pref);
        
        $id = $prefacturas[0]->prePaciente;
        $opciones[0]=$opciones[1]=$opciones[2]=$opciones[3]=$opciones[4]=$opciones[5]=$opciones[6]=$opciones[7]=$opciones[8]=$opciones[9]=$opciones[10]=$opciones[11]=$opciones[12]=$opciones[13]= '';
        $opciones[12]       = 'on';
        $datos              = $this->ingreso_model->dameTodo($id);
        $depDescuento = $depHonorario = 0;
        
        IF($prefacturas[0]->preGes === '2'){
            $depositos  = $this->bancos_model->dameTodosDeposito($prefacturas[0]->preRegistro);
            FOREACH($depositos as $dep){
                
                IF($dep->depConcepto==='9')$depDescuento += $dep->depSuma;
                IF($dep->depConcepto==='10')$depHonorario += $dep->depSuma;
            }
        }
        $data['depDescuento'] = $depDescuento;
        $data['depHonorario'] = $depHonorario;

        $data['insumosAsignados'] = $this->enfermeria_model->dameInsumosAsignados($datos[0]->id);
        //die(var_dump($data['insumosAsignados']));
        $data['datos']      = $datos[0];
        
        //die(($datos[0]->rutTitular));
        $pac = $this->pacientes_model->dameUno($datos[0]->rutTitular);
        $apo = $this->apoderados_model->dameUno($datos[0]->rutTitular);
            
        $titular[0]=$pac;
        $titular[1]=$apo;
        //die(var_dump($titular[0]));
        $data['datosApo']   = $titular;
        //die(var_dump($apo));
        $data['prefacturas'] = $prefacturas;
        //die(var_dump($data['datosApo']));
        //die(var_dump($data['datosApo']));
        $data['opciones']   = $opciones;
        $data['imprimir']   = 'imprimir';
        $data['breadcumb']  = "Ingreso";
    ...
    public function exportarPrefactura($pref)
    {
        $prefacturas = $this->prefacturas_model->dameUnaPrefClinica($pref);
        
        $id = $prefacturas[0]->prePaciente;
        $opciones[0]=$opciones[1]=$opciones[2]=$opciones[3]=$opciones[4]=$opciones[5]=$opciones[6]=$opciones[7]=$opciones[8]=$opciones[9]=$opciones[10]=$opciones[11]= '';
        $opciones[12]       = 'on';
        $datos              = $this->ingreso_model->dameTodo($id);
        $depDescuento = $depHonorario = 0;
        ...
        $data['breadcumb']  = "Ingreso";
        $data['title']      = "Ficha de Ingreso";
        Layout_Helper::cargaVista($this,'exportarPrefactura',$data,'ingresos');   
    }
    
    public function exportarDiaCama($pref)
    {
        $prefacturas = $this->prefacturas_model->dameUnaPrefClinica($pref);
        ...
        $data['valorCama']                 = $this->parametros_model->dameUno('4');
        $data['valorConsulta']            = $this->parametros_model->dameUno('8');
        $data['insumosAsignados'] = $this->enfermeria_model->dameInsumosAsignados($datos[0]->id);
        //die(var_dump($data['insumosAsignados']));
        $data['datos']      = $datos[0];
        ...
        $data['breadcumb']  = "Ingreso";
        $data['title']      = "Ficha de Ingreso";
        Layout_Helper::cargaVista($this,'exportarDiaCama',$data,'ingresos');   
    }
